---
title: "COPS - multi-omic TCGA breast cancer patient stratification"
...
weighs features based on the degree of of their corresponding node in a given 
pathway network:
$$
...
mopw_clusters <- COPS::multi_omic_clustering(
  ...
  mkkm_mr_mosek = FALSE,
  mkkm_mr_lambda = 1, 
  kernels = rep("BWK", length(dat$omics)),
  pathway_networks = pathway_networks,
  mvc_threads = 6,
  n_clusters = 2:8)
...
omics_d <- list()
omics_s <- list()
for (o in names(dat$omics)) {
  omics_d[[o]] <- COPS::clustering_dissimilarity_from_data(t(dat$omics[[o]]))
  omics_s[[o]] <- COPS::clustering_metrics(mopw_clusters, 
                                           dissimilarity = omics_d[[o]])$metrics
  omics_s[[o]]$metric <- paste0(o, "_", omics_s[[o]]$metric)
  omics_s[[o]] <- reshape2::dcast(omics_s[[o]], k + m ~ metric, value.var = "value")
}
...
molk_res <- COPS::COPS(
  ...
  mkkm_mr_mosek = FALSE,
  mkkm_mr_lambda = 1, 
  kernels = rep("linear", length(dat$omics)),
  mvc_threads = 6,
  n_clusters = 2:8)
```
...
```{r}
clinical_data <- TCGAbiolinks::TCGAquery_subtype("BRCA")
```

The <code>cv_survival_evaluation</code> expects a data.frame that contains the 
...
```{r}
surv_data <- COPS::survival_preprocess(event_data = clinical_data, 
...
After preprocessing survival data we can call <code>cv_survival_evaluation</code> 
with the clustering result and event data. It may be necessary to translate the 
sample IDs used in the omics data into patient IDs. Note that missing survival 
data is automatically omitted in the evaluation. <code>COPS</code> fits two 
Cox proportional hazard models one with cluster indicators and one without and 
...
such as age or cancer stage. The function retuns a data.frame summarising the 
LRT p-value and survival concordance index for each result. 

```{r}
mopw_clusters_surv <- mopw_clusters
mopw_clusters_surv[["id"]] <- gsub("-[0-9]+[A-Z]+$", "", mopw_clusters_surv[["id"]])
mopw_surv_res <- COPS::cv_survival_evaluation(event_data = surv_data, 
                                              clusters = mopw_clusters_surv,
                                              survival_time_col = "time",
                                              survival_event_col = "event", 
                                              survival_covariate_names = c("age_at_initial_pathologic_diagnosis", 
                                                                           "pathologic_stage"),
                                              row_id = "patient")
```
...
molk_clusters_surv[["id"]] <- gsub("-[0-9]+[A-Z]+$", "", molk_clusters_surv[["id"]])
molk_surv_res <- COPS::cv_survival_evaluation(event_data = surv_data, 
...
for (i in names(COPS::brca_multi_omics)) {
  mopw_features[[i]] <- COPS::univariate_features(COPS::brca_multi_omics[[i]], 
                                                  mopw_clusters[mopw_clusters[["k"]] == 3, 
                                                                "cluster"], # same order as input rows
                                                  parallel = 6)
}
```
...
for (i in names(COPS::brca_multi_omics)) {
  mopw_features_selected[[i]] <- which(apply(abs(mopw_features[[i]][["snr"]]) > 0.75, 1, any))
}
```
...
hm_annotations <- clinical_data[match(p_id, clinical_data[["patient"]]),
                                c("age_at_initial_pathologic_diagnosis",
                                  "pathologic_stage", "BRCA_Subtype_PAM50")]
...
COPS::heatmap_annotated(mopw_dat_selected, hm_annotations, scale = TRUE,
