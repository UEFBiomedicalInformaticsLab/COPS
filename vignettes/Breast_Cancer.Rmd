...
data is automatically omitted in the evaluation. <code>cv_survival_evaluation</code> 
...
for (i in names(COPS::brca_multi_omics)) {
  mopw_features_selected[[i]] <- names(which(apply(abs(mopw_features[[i]][["snr"]]) > 0.75, 1, any)))
}
```
...
COPS::heatmap_annotated(mopw_dat_selected, 
                        hm_annotations, 
                        scale = TRUE,
