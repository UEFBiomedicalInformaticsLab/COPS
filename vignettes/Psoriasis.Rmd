---
title: "COPS tutorial, gene-expression-driven psoriasis patient stratification"
...
from multiple studies collected by Federico et al. (2020). The full dataset 
including atopic dermatitis studies is available at Zenodo. We also only consider 
protein coding genes in this tutorial. 
...
```{r}
scores <- COPS::clusteval_scoring(res, wsum = TrainStabilityJaccard + 
                                              Silhouette - GSE.nmi,
...
COPS::pareto_plot(scores$all[scores$all[["Smallest_cluster_size"]] >= 10,],
                  metrics = c("TrainStabilityJaccard", "Silhouette", 
...
Feature selection has a very large impact on the results. Here we present 
results obtained by using a preselected set of genes

```{r}
deg_dist <- dist(t(log2(pso_combat_normalized[COPS::psoriasis_degs_example,lesional_id]+1)))
res_degs <- COPS::COPS(log2(pso_combat_normalized[COPS::psoriasis_degs_example,lesional_id]+1), 
                       parallel = 6, nruns = 2, nfolds = 5, 
                       dimred_methods = c("pca"), 
                       pca_dims = c(2:10),
                       cluster_methods = c("diana", "kmeans"), 
                       ...
                       n_clusters = 2:6,
                       association_data = psoriasis_cli[lesional_id,],
                       silhouette_dissimilarity = deg_dist)
```

```{r, out.width="100%", fig.width = 8, fig.height = 6}
scores <- COPS::clusteval_scoring(res_degs, wsum = TrainStabilityJaccard + 
                                              Silhouette - GSE.nmi,
                                  significance_level = 0.05)
COPS::pareto_plot(scores$all[scores$all[["Smallest_cluster_size"]] >= 10,],
                  metrics = c("TrainStabilityJaccard", "Silhouette", 
