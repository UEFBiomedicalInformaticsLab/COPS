...
RWRFGSEA <- function(expr, 
  ...
  if (!is.null(rwr_gene_lists)) {
    if (length(rwr_gene_lists) == 2) {
      expr_list <- list(expr[rwr_gene_lists[[1]], ], expr[rwr_gene_lists[[2]], ])
    } else {
  ...
  for (i in 1:length(expr_list)) {
    ...
    if (!is.null(disease_genes)) {
      expr_list[[i]] <- expr_list[[i]][intersect(rownames(expr_list[[i]]), disease_genes),]
    }
  ...
  rwrfgsea_results <- t(COPS::fgsea_wrapper(rwr_results, 
                                            gene_set_list, 
                                            parallel = parallel, 
...
#'
#' @export
fgsea_wrapper <- function(data_matrix, 
                          gene_set_list, 
                          fgsea_input_cutoff = 0, 
                          parallel = 1, 
                          fgsea_nperm = 10000, 
  ...
  res <- tryCatch(foreach(genes_i = lapply(1:ncol(data_matrix), function(i) data_matrix[,i]), 
                 ...
                 .maxcombine = max(ncol(data_matrix), 2)) %dopar% {
                   ...
                   res_i <- fgsea::fgseaSimple(gene_set_list, 
                                               genes_i, 
                                               nperm = fgsea_nperm, 
