...
if (FALSE) {
  ...
  ## Old 1
  temp <- as.data.frame(t(1:ncol(brca_silhouette_table)))
  colnames(temp) <- colnames(brca_silhouette_table)
  
  brca_silhouette_table <- rbind(temp, brca_silhouette_table)
  brca_silhouette_table <- rbind(temp, brca_silhouette_table)
  brca_silhouette_table <- rbind(temp, brca_silhouette_table)
  
  brca_silhouette_table[1,] <- c("", "", sapply(strsplit(colnames(brca_silhouette_table), "_")[-(1:2)], 
                                                function(x) x[[length(x) - 1]]))
  brca_silhouette_table[2,] <- c("", "", sapply(strsplit(colnames(brca_silhouette_table), "_")[-(1:2)], 
                                                function(x) paste0("k=", x[[length(x)]])))
  brca_silhouette_table[3,] <- c("", "", sapply(strsplit(colnames(brca_silhouette_table), "_")[-(1:2)], 
                                                function(x) ifelse(x[[length(x) - 2]] == "sd", "sd", "mean")))
  
  ind <- order(brca_silhouette_table[1,-(1:2)], brca_silhouette_table[2,-(1:2)], brca_silhouette_table[3,-(1:2)])
  
  write.table(brca_silhouette_table[,c(1,2, 2 + ind)], 
  ...
  ## Old 2
  # Create tables
  brca_silhouette_table <- tidyr::pivot_wider(brca_table, 
                                              id_cols = c("Embedding", "Clustering"),
                                              names_from = c("Approach", "k"),
                                              #names_prefix = "k",
                                              names_sep = "_",
                                              values_from = c("Silhouette", "Silhouette_sd"))
  #ind <- (2:(ncol(brca_silhouette_table) %/% 2)) * 2
  
  #write.table(brca_silhouette_table[,c(1,2, 2 + order(c(ind - 1, ind)))], 
  #            file = paste0(path_plots, "/brca_silh.csv"), 
