...
brca_survival_pw <- ggplot(brca_bp_quantiles[brca_bp_quantiles$Embedding %in% c("GO", "KEGG", "REACTOME", "Hallmark"),], 
...
prad_survival_pw <- ggplot(prad_bp_quantiles[prad_bp_quantiles$Embedding %in% c("GO", "KEGG", "REACTOME", "Hallmark"),], 
              aes(x = k, fill = Embedding)) + 
  geom_boxplot(aes(lower = Q025, upper = Q075, middle = Q05, ymin = ymin, ymax = ymax), 
               outlier.shape = NA, stat = "identity", lwd = 0.25) + 
  theme_bw() + scale_fill_brewer(palette = "Dark2") + 
