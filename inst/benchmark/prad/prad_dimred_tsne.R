...
DIMRED_METHODS <- c("tsne")

# Run pipeline for all non-zero variance genes
res_tsne <- COPS::dimred_clusteval_pipeline(list(expr = log2(tprad_norm[-zero_var,] + 1)), 
...
scores_tsne <- COPS::clusteval_scoring(res_tsne, wsum = (NMI.Gleason_category + 1 - NMI.tss) / 2, summarise = SUMMARISE)
write.csv(scores_tsne$all, paste0(path_intermediate_results, "/prad/dimred/dimred_tsne/scores.csv"))
write.csv(res_tsne$clusters, gzfile(paste0(path_intermediate_results, "/prad/dimred/dimred_tsne/clusters.csv.gz")))
