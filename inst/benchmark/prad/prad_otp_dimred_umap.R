...
# UMAP 10 neighbours
res_umap10_otp <- COPS::dimred_clusteval_pipeline(list(expr = log2(tprad_norm[combined_gene_filter,] + 1)), 
...
write.csv(res_umap10_otp$clusters, gzfile(paste0(path_intermediate_results, "/prad/dimred_otp/dimred_umap_10n/clusters.csv.gz")))

# UMAP 20 neighbours
res_umap20_otp <- COPS::dimred_clusteval_pipeline(list(expr = log2(tprad_norm[combined_gene_filter,] + 1)), 
                                                  batch_label = prad_norm_batch, 
                                                  subtype_label = prad_subtype, 
                                                  parallel = PARALLEL, nruns = NRUNS, 
                                                  nfolds = NFOLDS, dimred_methods = DIMRED_METHODS,
...
write.csv(res_umap20_otp$clusters, gzfile(paste0(path_intermediate_results, "/prad/dimred_otp/dimred_umap_20n/clusters.csv.gz")))

# UMAP 20 neighbours
res_umap30_otp <- COPS::dimred_clusteval_pipeline(list(expr = log2(tprad_norm[combined_gene_filter,] + 1)), 
                                                  batch_label = prad_norm_batch, 
                                                  subtype_label = prad_subtype, 
                                                  parallel = PARALLEL, nruns = NRUNS, 
                                                  nfolds = NFOLDS, dimred_methods = DIMRED_METHODS,
...
write.csv(res_umap30_otp$clusters, gzfile(paste0(path_intermediate_results, "/prad/dimred_otp/dimred_umap_30n/clusters.csv.gz")))
