...
res_umap30 <- COPS::dimred_clusteval_pipeline(list(expr = log2(tbrca_norm[-zero_var,] + 1)), 
...
scores_umap30 <- COPS::clusteval_scoring(res_umap30, wsum = (NMI.BRCA_Subtype_PAM50 + 1 - NMI.tss) / 2, summarise = SUMMARISE)
write.csv(scores_umap30$all, paste0(path_intermediate_results, "/brca/dimred/dimred_umap_30n/scores.csv"))
write.csv(res_umap30$clusters, gzfile(paste0(path_intermediate_results, "/brca/dimred/dimred_umap_30n/clusters.csv.gz")))
