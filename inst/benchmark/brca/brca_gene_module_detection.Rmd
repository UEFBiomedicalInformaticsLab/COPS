---
title: "BRCA co-expression network inference and module detection"
...
source("brca_default_parameters.R")
source("tcga_brca_mrna_data.R")
```
...
adj <- WGCNA::adjacency.fromSimilarity(gene_correlation, power = 6)
...
```{r}
dissTOM_all <- dissTOM
adj_modules <- WGCNA::adjacency.fromSimilarity(gene_correlation[dynamicColors != "grey", dynamicColors != "grey"], power = 6)
...
MEList = WGCNA::moduleEigengenes(t(log2(tbrca_norm[gene_filter,][secondary_gene_filter,] + 1)), colors = dynamicColors[secondary_gene_filter])
