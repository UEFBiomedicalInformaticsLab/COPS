...
res_umap20 <- COPS::dimred_clusteval_pipeline(list(expr = log2(tbrca_norm[-zero_var,] + 1)), 
...
scores_umap20 <- COPS::clusteval_scoring(res_umap20, wsum = (NMI.BRCA_Subtype_PAM50 + 1 - NMI.tss) / 2, summarise = SUMMARISE)
write.csv(scores_umap20$all, paste0(path_intermediate_results, "/brca/dimred/dimred_umap_20n/scores.csv"))
write.csv(res_umap20$clusters, gzfile(paste0(path_intermediate_results, "/brca/dimred/dimred_umap_20n/clusters.csv.gz")))
