...
dat_list <- list(expr = log2(tprad_norm[combined_gene_filter,]+1))
...
# Survival evaluation
clust_survival <- COPS::survival_evaluation(prad_survival, clusters, by = c("run", "fold", "k", "m"), 
...
scores_sc3 <- COPS::clusteval_scoring(out, wsum = (NMI.Gleason_category + 1 - NMI.tss) / 2, summarise = SUMMARISE)
write.csv(scores_sc3$all, paste0(path_intermediate_results, "/prad/sc3/scores.csv"))
write.csv(clusters, gzfile(paste0(path_intermediate_results, "/prad/sc3/clusters.csv.gz")))
